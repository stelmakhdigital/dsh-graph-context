...
async function handlePreStep(payload, next, config, deps, runGraph2) {
  ...
  let result = config.reinjectAfterCompaction ? await reInjectCompactedMap(payload, config, deps, runGraph2, decision) : decision;
...
async function handleAgentCreated(agent, config, deps, runGraph2) {
...
function registerHooks(ctx, config, deps, runGraph2) {
  ...
  if (config.injectSubagentMap) {
    ctx.on("agent/created", (payload) => {
      void handleAgentCreated(payload.agent, config, deps, runGraph2).catch((error) => {
