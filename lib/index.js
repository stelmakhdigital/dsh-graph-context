...
async function handleSessionStart(agent, source, config, deps, runGraph2) {
  ...
  if (config.blastOnResume && source === "resume" && deps.gitDirty !== void 0) {
    ...
    try {
      ...
      if (dirty) {
        const { json, code } = await runGraph2(["blast", "--depth", "2", "--format", "json", blastRoot], {
          cwd: blastRoot,
          timeoutMs: config.timeoutMs,
          graphPath: config.graphPath
        });
...
async function handlePreStep(payload, next, config, deps, runGraph2) {
  ...
  if (config.injectSubagentMap) {
    result = await injectSubagentMap(payload, config, deps, runGraph2, result);
  }
  if (config.reinjectAfterCompaction) {
    result = await reInjectCompactedMap(payload, config, deps, runGraph2, result);
  }
...
async function reInjectCompactedMap(payload, config, deps, runGraph2, decision) {
  try {
    const agent = payload.agent;
    const anchor = sessionAnchor(agent, deps);
    if (anchor === void 0 || anchor.outsideGit) return decision;
...
async function injectSubagentMap(payload, config, deps, runGraph2, decision) {
  try {
    const agent = payload.agent;
    const anchor = sessionAnchor(agent, deps);
    if (anchor === void 0 || anchor.outsideGit) return decision;
