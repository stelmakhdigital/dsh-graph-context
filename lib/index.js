...
async function handleSessionStart(agent, source, config, deps, runGraph3) {
...
function registerHooks(ctx, config, deps, runGraph3) {
  if (config.injectSessionMap) {
    ctx.on("agent/session-start", (payload) => {
      void handleSessionStart(payload.agent, payload.source, config, deps, runGraph3).catch((error) => {
