...
Обновлено: 2026-09-14 · Статус: **v0.6.0 выпущен** (P2c-старт: #3 graph_blast + resume-blast); **стоп до остатка P2c** (change control)
...
| P2c (тяжёлые фичи) | 🟡 2026-09-15 | M11 | **#3 ✅ (v0.6.0, live-проверен 2026-09-15)**: tool `graph_blast` (`graft blast --format json`, сверено с CLI 0.18.0: `--base`/`--depth`) + resume-blast (session-start `resume` + dirty git → короткий blast, флаг `blastOnResume` default on); 219 тестов + e2e 3/3. **Live (GUI на 0.6.0)**: субагент внёс правку в `blastSymbolLine` → `graph_blast` без аргументов вернул `basis=working tree vs HEAD`, `seeds=[blastSymbolLine]`, `impacted=[renderBlast (calls, depth 1)]` — ровно соответствует графу; SHORT_MAP субагента снова ✅. Остаток: #4 Code Mode, #7 graph_enrich (--deep local), #17 watcher — проверка API хоста |
