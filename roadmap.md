...
| P2c (тяжёлые фичи) | 🟡 2026-09-14 | M11 | **#3 ✅ (v0.6.0)**: tool `graph_blast` (`graft blast --format json`, сверено с CLI 0.18.0: `--base`/`--depth`) + resume-blast (session-start `resume` + dirty git → короткий blast, флаг `blastOnResume` default on); 219 тестов + e2e 3/3. Остаток: #4 Code Mode, #7 graph_enrich (--deep local), #17 watcher — проверка API хоста |
