...
- Статус-кво: 199 юнит-тестов (TDD, вкл. api-guard) + opt-in e2e 3/3 с реальным CLI (fixture `/tmp/graph-sample`); typecheck strict (`noUncheckedIndexedAccess` — индексные доступы дают `T | undefined`); `lib/` собран esbuild (коммитится); `dsh-context-graph-0.5.0.tgz` в web+cg.
- Артефакты: `dsh-context-graph-0.5.0.tgz`; профили: `web` и `cg` (headless-шаблон) с плагином 0.5.0; `~/.dsh/skills/graph/SKILL.md` установлен (старый `~/.dsh/skills/graft/` удалён).
- Live (2026-09-13): web-бут 0.5.0 (3199, tokenized URL отдаёт 303→app), headless `cg` exit 0. Плагин живьём работает в этой GUI-сессии (session-start-мап, pre-step prompt hits, post-edit blast radius).
