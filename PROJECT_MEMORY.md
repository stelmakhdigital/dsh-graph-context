...
- Фаза: **P2b завершён (v0.5.2)** (roadmap.md P2b-строка ✅ M10, live-фиксы 0.5.1/0.5.2). P2b-пакет: #5 subagent-карта (`agent/created` + guard `session.header.origin === 'subagent'` — **не** `agent.parentAgent`, см. ниже; короткий мап, бюджет `maxInjectBytes/2`, доставка через pre-step decision), #6 compaction re-inject (`session/event` → `compaction/summary|prune`, одноразовый мап на следующем pre-step), #11 toolOrder (waterfall `system-prompt/assemble` → graph-tools первыми, no-op без события). Три новых флага конфига: `injectSubagentMap` / `reinjectAfterCompaction` / `toolOrder` (default on).
...
- Статус-кво: 203 юнит-теста (TDD, вкл. api-guard) + opt-in e2e 3/3 с реальным CLI (fixture `/tmp/graph-sample`); typecheck strict (`noUncheckedIndexedAccess` — индексные доступы дают `T | undefined`); `lib/` собран esbuild (коммитится); `dsh-context-graph-0.5.2.tgz` в web+cg (live: web-бут + headless exit 0).
- Артефакты: `dsh-context-graph-0.5.2.tgz`; профили: `web` и `cg` (headless-шаблон) с плагином 0.5.2; `~/.dsh/skills/graph/SKILL.md` установлен (старый `~/.dsh/skills/graft/` удалён).
...
- Важно: GUI (:3080) работает на bundle'е, загруженном при последнем рестарте — чтобы подхватить **0.5.2**, нужен ещё один рестарт web-сервера (процесс стартовал 01:31 с 0.5.1).
