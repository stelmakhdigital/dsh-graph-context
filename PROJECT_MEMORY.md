...
- Фаза: **P2a завершён (v0.3.0)** (roadmap.md фазы 8–9 ✅, 2026-09-13). P2a-пакет: #1 `injectMode` (sourced/pointers/map-only), #2 blind-search nudge (reminder, не ban), #8 `scopeFromLastEdit` (`--in` из rel-paths), #12 wiring-read guard (opt-in deny), #16 локальные metrics, #14 invariant-тест (no fetch/http), доки #13/#15 в README EN/RU. Новый хук `tools/pre-execute` (waterfall) — контракт сверен с packages/core/tools + hooks-claude-code. Отчёт — roadmap §«Отчёт P2a».
- Инцидент 2026-09-13: 0.3.0 не грузился — `z.enum is not a function` (R9, stub-drift: `enum` была выдумана в stub+мок, реального schemastery-метода нет). **Исправлено и проверено**: `z.union([z.const…])`, guard-тест, 175 unit + e2e 3/3, standalone-смоук на реальном schemastery 3.18.2, `pnpm dsh web` бут зелёный (port 3199, app отдаёт страницу, skill установлен apply()), headless-прогон `cg` exit 0.
- Статус-кво: 175 юнит-тестов (TDD, вкл. api-guard) + opt-in e2e 3/3 с реальным CLI (fixture `/tmp/graph-sample` пересобран); typecheck strict; `lib/` собран esbuild (коммитится); `dsh-context-graph-0.3.0.tgz` упакован.
- Артефакты: `dsh-context-graph-0.3.0.tgz`; профили: `web` — плагин 0.3.0 переустановлен (после переустановки конфигурации владельцем plugin-регистрации были стёрты — bundles `[]`, node_modules пуст), `cg` — воссоздан из headless-шаблона + плагин 0.3.0; `~/.dsh/skills/graph/SKILL.md` установлен.
- Важно: бегущий web-GUI (:3080) поднят после переустановки конфигурации БЕЗ плагина — **нужен рестарт web-сервера**, чтобы подхватить 0.3.0 (иначе в GUI графа нет; headless-профиль `cg` работает с плагином сразу).
- Следующее: **СТОП** — ждём владельца. P2b (#5/#6/#11) и P2c (#3/#4/#7/#17) — только по явной команде (change control, Won't-список roadmap).
