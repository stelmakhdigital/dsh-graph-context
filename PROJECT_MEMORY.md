...
- Фаза: **P2b завершён (v0.5.2), P2c начат (v0.6.0)**: #3 graph_blast ✅ (7-й tool `graph_blast` + resume-blast при session-start `resume` на dirty git, флаг `blastOnResume` default on; `graft blast` сверён с CLI 0.18.0). Остаток P2c: #4 Code Mode, #7 local --deep, #17 watcher. P2b-пакет: #5 subagent-карта (`agent/created` + guard `session.header.origin === 'subagent'` — **не** `agent.parentAgent`, см. ниже; доставка через pre-step decision), #6 compaction re-inject, #11 toolOrder. Флаги: `injectSubagentMap` / `reinjectAfterCompaction` / `toolOrder` / `blastOnResume` (default on).
...
- Статус-кво: 219 юнит-тестов (TDD, вкл. api-guard) + opt-in e2e 3/3 с реальным CLI (fixture `/tmp/graph-sample` — **восстановлен 2026-09-14** с символом `issueToken`); typecheck strict (`noUncheckedIndexedAccess` — индексные доступы дают `T | undefined`); `lib/` собран esbuild (коммитится); `dsh-context-graph-0.6.0.tgz` в web+cg (live: web-бут 3199 + headless OK).
- Артефакты: `dsh-context-graph-0.6.0.tgz`; профили: `web` и `cg` (headless-шаблон) с плагином 0.6.0; `~/.dsh/skills/graph/SKILL.md` установлен (обновится через installSkill-хук на следующих сессиях: +строка `graph_blast`).
...
- Важно: GUI (:3080) теперь работает на **0.6.0** (процесс стартовал 15:56 2026-09-15, после установки 0.6.0 в 15:38).
...
- Следующее: **СТОП** — ждём владельца. Остаток P2c: #4 Code Mode, #7 graph_enrich (--deep local), #17 watcher — только по явной команде (change control, Won't-список roadmap).
