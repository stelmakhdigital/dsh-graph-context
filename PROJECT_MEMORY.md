...
- Важно: GUI (:3080) работает на bundle'е, загруженном при последнем рестарте — чтобы подхватить **0.6.0**, нужен ещё один рестарт web-сервера.
- **Live-проверка 0.6.0 (после рестарта GUI)**: (1) субагент-чек `graph_blast`: «вызови graph_blast на этом репо и перешли blastText» (репо dirty — незакоммиченные правки → populated seeds/impacted); (2) resume-blast: резюм сессия с грязным деревом → блок «Blast radius (working tree vs HEAD…)» в начале контекста.
